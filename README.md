...
```
GET http://175.45.201.103/answer?question_id={질의 ID}&question={평가 질의}
```

- 표준 HTTP 포트(80)로 구성했으며 별도 도메인 없이 공인 IP로 접근한다.
- 응답은 `question_id`, `question`, `retrieved_context`, `think_trace`, `answer` 5개 키를 가진 JSON이며 모든 값은 문자열이다.
