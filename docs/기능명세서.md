...
| 평가·오케스트레이션 | Controller·Service·QuestionPlan·Retriever 인터페이스의 초기 뼈대 |
...
현재 `EvaluationController`는 `/api/v1/answer`에 가까운 뼈대이며 실제 질문 처리를 하지 않는다. 제출 목표 경로 `GET /answer`와는 다르므로 P0 연결 단계에서 수정한다.
...
HCX가 만든 후보와 백엔드가 승인한 계획은 같은 스키마를 사용한다.
...
검증 결과는 계획 바깥의 별도 객체로 관리한다.

```json
{
  "status": "VALID",
  "normalizedFields": ["companies[0].companyId"],
  "warnings": []
}
```
...
목표 계약을 도입할 때 JDK·Actuator 타입이 아닌 자체 도메인 enum과 step DTO를 사용한다.
...
평가 API는 동기 응답이지만 내부 단계와 처리시간은 기록할 수 있어야 한다. P1 웹은 같은 상태를 사용자용 문구로 변환한다.
...
| `question_runs` | 질문 실행 상태·버전 | 평가 코어 기록 |
