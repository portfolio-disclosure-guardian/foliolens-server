...
현재 `EvaluationAnswerController`는 제출 목표 경로 `GET /answer`에서 `question_id`와 `question`을 받아 공통 서비스로 전달한다. 다만 `DisclosureAnswerService`는 `QuestionRun`을 `PENDING`으로 생성한 뒤 검색·계산·HCX·검증 없이 placeholder 답변을 반환하고, 평가 mapper는 `retrieved_context`를 항상 빈 배열로 만든다.
...
- 접수기간·보고기간·기준시점을 구분하지 않는 단일 `Instant`
- 후보의 단일 회사 문자열과 단순 `Instant from/to`
